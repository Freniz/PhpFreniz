...
<?php
    $output;
   if(isset($_REQUEST['imageid']) && isset($_REQUEST['deletesrc']) && isset($_REQUEST['secpicno']) && isset($_REQUEST['x']) && isset($_REQUEST['y']) && isset($_REQUEST['width']) && isset($_REQUEST['height']) && isset($_SESSION['userid'])){
       ...
       else if($_SESSION['userid']==$suserid || $_SESSION['userid']==$ruserid && in_array($pinnedpeople, $_SESSION['userid']))
       {
           ...
           mysql_query("insert into image (title,description,url,albumid,userid,date,pinnedpeople,vote,pt,specificlist,hiddenlist,notifyusers,accepted,reqpinusers,pinmereq,comments) values('title','description','".$filename.'.'.$ext."','".$_SESSION['userdetails']['secondarypicalbum']."','".$_SESSION['userid']."',now(),'a:0:{}','a:0:{}','public','a:0:{}','a:0:{}','a:0:{}','yes','a:0:{}','a:0:{}','a:0:{}')");
           $updtid=mysql_insert_id();
           if($_REQUEST['secpicno']==1)
            mysql_query ("update user_info set secondarypic1='".$updtid."' where userid='".$_SESSION['userid']."'");
            else
                mysql_query ("update user_info set secondarypic2='".$updtid."' where userid='".$_SESSION['userid']."'");
       }

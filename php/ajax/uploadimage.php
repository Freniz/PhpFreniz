...
class qqUploadedFileXhr {
    ...
    function save($path) {    
        ...
        $c=new compressimage($path,$resolutions);
        return true;
    }
    function getName() {
        return $_GET['qqfile'];
...
class qqFileUploader {
     ...
     */
    function handleUpload($uploadDirectory,$pt,$specific,$hidden,$ruser,$accepted,$replaceOldFile = FALSE){
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
            ...
            if($accepted=='yes'){
                ...
                if($count==0)
                mysql_query("insert into activity (userid,ruserid,contentid,title,contenttype,contenturl,date) values ('".$_SESSION['userid']."','".$ruser."','".$updtdid."','post image','image','image.php?imageid=".$updtdid."',now())");
                else{
                    mysql_query ("insert into activity (userid,ruserid,contentid,title,contenttype,contenturl,date) values ('".$_SESSION['userid']."','".$ruser."','".$_REQUEST['album']."','posted $count images','album','album.php?albumid=".$_REQUEST['album']."',now())");
                }
            ...
            fwrite($target, "\n".serialize($results));
            fclose($target);
            return array('success'=>"true","fileid"=>$updtdid,"imgurl"=>($filename.".".$ext));
        } else {
